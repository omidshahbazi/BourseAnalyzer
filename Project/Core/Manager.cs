...
namespace Core
{
	public class Manager
	{
		...
		public void Run1()
		{
			...
			{
				//TODO: revise manager
				//TODO: do the analyze with history
				StringBuilder query = new StringBuilder();
				...
				for (int i = 0; i < workerDatesData.Rows.Count; ++i)
				{
					...
					for (int j = 0; j < workers.Length; ++j)
					{
						...
						DataTable workerSchedulesData = Data.QueryDataTable("SELECT id, schedule_time FROM worker_schedules WHERE name=@name AND done=0", "name", workerName);
						for (int k = 0; k < workerSchedulesData.Rows.Count; ++k)
						{
							...
							if (scheduleTime.Date != date || scheduleTime > nowTime)
								continue;
